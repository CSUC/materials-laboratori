...
//					error_log("BINDING: " . print_r ($binding, true));
...
//		error_log("wherelog: " . $where . "binding[0]: " + $binding[0]);
		return $where;
	}
...
/*		$query = "SELECT `".implode("`, `", self::pluck($columns, 'db'))."`
		 ...
		 $limit";
*/
//		 error_log("bindigs: " . print_r($binding, true));
//		error_log($query);
	...
	static function sql_exec ( $db, $bindings, $sql=null )
	{
		...
		if ( is_array( $bindings ) ) {
			for ( $i=0, $ien=count($bindings) ; $i<$ien ; $i++ ) {
				$binding = $bindings[$i];
//				error_log("bindingEXEC: key" .  $binding['key'] . "  val: " . $binding['val']  . " type: " . $binding['type']);
